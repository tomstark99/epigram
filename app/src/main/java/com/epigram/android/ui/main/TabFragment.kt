...
class TabFragment : BaseFragment<TabMvp.Presenter>(), TabMvp.View, LoadNextPage {
    ...
    companion object {
        ...
        const val ARG_HAS_BREAKING = "has_breaking.bool"
    ...
    override fun onPostSuccessHome(posts: List<Post>) {
        pageNum++
        if(adapterHome == null) {
            adapterHome = AdapterArticlesHome(context!!, emptyList<Post>().toMutableList(), posts.toMutableList(), this, tabNum)
    ...
    override fun onPostSuccessCorona(corona: List<Post>, posts: List<Post>) {
        pageNum++
        if(adapterHome == null) {
            adapterHome = AdapterArticlesHome(context!!, corona.toMutableList(), posts.toMutableList(), this, tabNum)
            my_recycler_view.adapter = adapterHome
        } else {
            if(pageNum == FIRST_INDEX + 1) (adapterHome as AdapterArticlesHome).clear()
            adapterHome!!.addPosts(posts)
